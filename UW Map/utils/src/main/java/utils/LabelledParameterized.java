...
public class LabelledParameterized extends Parameterized {
  ...
  private void initialiseLabels() throws Exception {
    ...
    for (FrameworkMethod junitMethod : methods) {
      ...
      if (!Modifier.isStatic(modifiers)) throw new Exception("@Labels method must be static");

      Class<?> returnType = method.getReturnType();
      if (returnType != List.class)
        throw new Exception("Return type of @Labels method must be List<String>");
  ...
  /**
   * Relies on JUnit 4.4. Can't use childlessCopy() method that features in later versions.
   *
   * <p>Relies on childDescriptions list being modifiable.
   */
